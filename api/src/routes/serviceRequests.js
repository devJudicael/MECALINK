...
router.post('/', auth, checkRole(['client']), createRequestValidation, async (req, res) => {
  ...
  try {
    const { garageId, description, location, vehicleInfo, urgency } = req.body;
...
router.get('/client', auth, checkRole(['client']), async (req, res) => {
  try {
    const serviceRequests = await ServiceRequest.find({ clientId: req.user._id })
      .sort({ createdAt: -1 });
...
router.get('/garage', auth, checkRole(['garage']), async (req, res) => {
  try {
    ...
    const serviceRequests = await ServiceRequest.find({ garageId: garage._id })
      .sort({ createdAt: -1 });
...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    if (
      req.user.role === 'client' && serviceRequest.clientId.toString() !== req.user._id.toString() ||
      req.user.role === 'garage' && serviceRequest.garageId.toString() !== (await Garage.findOne({ userId: req.user._id }))._id.toString()
    ) {
...
router.patch('/:id/status', auth, checkRole(['garage']), [
  body('status').isIn(['accepted', 'rejected', 'completed']).withMessage('Statut invalide')
], async (req, res) => {
  // Vérifier les erreurs de validation
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
...
router.patch('/:id/cancel', auth, checkRole(['client']), async (req, res) => {
  try {
    ...
    if (serviceRequest.status === 'completed' || serviceRequest.status === 'cancelled') {
      return res.status(400).json({ message: 'Cette demande ne peut pas être annulée' });
    }
    ...
    res.status(500).json({ message: 'Erreur lors de l\'annulation de la demande', error: error.message });
  }
});
